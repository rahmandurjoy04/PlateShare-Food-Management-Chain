...
const MyPickups = () => {
  ...
  return (
    <div className="min-w-sm mx-auto p-4">
      <h2 className="text-3xl font-bold text-center mb-6 text-green-800">My Pickups</h2>

      {loading && <p className="text-center">Loading your pickups...</p>}

      {!loading && pickups.length === 0 && (
        <p className="text-center text-gray-500">No assigned pickups available.</p>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 min-w-sm">
        {pickups.map((pickup) => (
          <div key={pickup._id} className="border rounded-xl shadow p-4 bg-white">
